...
当前版本使用统一商户凭证。请求必须携带 `pid`，服务端用 `pid` 查询对应 API Key 并解密 `secret_key` 作为签名密钥。SQLite 中保存的是认证密文，不是可直接使用的明文；主密钥在数据库和镜像之外，详见 [API_KEY_SECRET.md](API_KEY_SECRET.md)。默认安装会创建一个 PID 为 `1000` 的默认密钥。
